...
extern volatile uint32_t buttonDis;
...
void ButtonISR(void) {
    ...
    uint32_t gpio_buttons = (~GPIOPinRead(GPIO_PORTD_BASE, 0xff) & (GPIO_PIN_4)) | (~GPIOPinRead(GPIO_PORTK_BASE, 0xff) & (GPIO_PIN_6)) >> 3 | (~GPIOPinRead(GPIO_PORTH_BASE, 0xff) & (GPIO_PIN_1)) << 1 |
            (~GPIOPinRead(GPIO_PORTJ_BASE, 0xff) & (GPIO_PIN_1 | GPIO_PIN_0)); // USR_SW1 & USR_SW2 buttons in positions 0 and 1, S1 & s2 buttons in position 2 and 3, Select button in position 4
    ...
    buttonDis = gButtons; //value to be displayed in main
